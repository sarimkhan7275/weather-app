...
export const SavedCities = ({ setCityName }: { setCityName: (cityName: string) => void }) => {
  ...
  return (
    ...
    <DropdownMenuContent className="bg-white/5 rounded-2xl border-none backdrop-blur-sm p-2 space-y-2" >
      ...
      {
        get_fav_city_loading ? <div className="flex items-center justify-center">
          <div className="h-12 w-12 animate-spin rounded-full border-4 border-solid border-teal-600 border-t-transparent"></div>
        </div>:get_fav_city_success?.cities?.length==0 ? <div className="text-white text-xs font-semibold py-4 px-2 " >No favourite city added ye!</div>:
        get_fav_city_success?.cities?.map((el:string, i:number)=>(
          <div key={`fav-cities-${i}`} className='flex justify-between items-center mx-auto rounded-full p-1 px-3 gap-x-4 text-white bg-white/15 backdrop-blur-2xl ' >
            <h3 onClick={()=>setCityName(el)}  className='text-center text-lg md:text-sm font-semibold ' >{capitalizeFirstLetter(el)}</h3>
            <div className='h-[40px] md:h-[30px] ' >
              <img className='h-full w-full object-contain ' src={`https://weather-app-rho-weld.vercel.app/assets/icons/02d.png`} alt="cloud-png" />
            </div>
